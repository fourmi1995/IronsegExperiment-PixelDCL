    ...
    def train(self):
        ...
        for epoch_num in range(self.conf.max_step+1):
            if epoch_num and epoch_num % self.conf.test_interval == 0:
                sig,inputs, labels = valid_reader.next_batch(self.conf.batch)
            ...
            if epoch_num and epoch_num % self.conf.summary_interval == 0:
                sig,inputs, labels = train_reader.next_batch(self.conf.batch)
                feed_dict = {self.inputs: inputs,
                             self.labels: labels}
    ...
    def predict(self):
        ...
        for index, prediction in enumerate(predictions):
            for i in range(prediction.shape[0]):
                ...
                scipy.misc.imsave(self.conf.sampledir +
                       str(index*prediction.shape[0]+i)+'.jpg', final_inputs[index][i])
                scipy.misc.imsave(self.conf.sampledir +
                       str(index*prediction.shape[0]+i)+'_label.png',final_labels[index][i]*45)
